<?php
    ...
    $action = $_GET['action'] ?? '';
    foreach($listIndexs as $key => $value){
        //thực hiện tạo và xóa index dựa vào listIndexsExits
        if($action == 'create' && $listIndexsExits[$key]){
            $client->indices()->create(['index' => $value]);
            $listIndexsExits[$key] = true;
            $listMessages[$key] = 'Tạo mới chỉ mục '.$value.' thành công';
        }else if($action == 'delete' && !$listIndexsExits[$key]){
            $client->indices()->delete(['index' => $value]);
            $listIndexsExits[$key] = false;
            $listMessages[$key] = 'Xóa chỉ mục '.$value.' thành công';
        }
    }
?>
<div class='card m-4'>
