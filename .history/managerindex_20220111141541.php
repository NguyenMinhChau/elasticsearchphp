    ...
    <div class='card-body'>
        <?php
            foreach ($listIndexs as $key => $value) {
                ...
                echo "<p>$listMessages[$key]</p>";
                echo '</div>';
                if(!$listMessages[$key]){
                    echo "<a href='http://localhost:8800/?page=managerindex&action=create$listsSlug[$key]' class='btn btn-primary'>Tạo chỉ mục $value</a>";
                    echo "<a href='http://localhost:8800/?page=managerindex&action=delete$listsSlug[$key]' class='btn btn-danger'>Xóa chỉ mục $value</a>";
                }
            }
        ?>
    </div>
