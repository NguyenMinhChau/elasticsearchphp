    ...
    <div class='card-body'>
        <?php
            foreach ($listIndexs as $key => $value) :?>
                ...
                <?php 
                    if(!$listIndexsExits[$key]) :?>
                        <a class='btn btn-primary' href='http://localhost:8800/?page=managerindex&action=createhealthcare_new'>Tạo chỉ mục <?=$value?></a>
                    <?php else: ?>
                        <a class='btn btn-danger' href='http://localhost:8800/?page=managerindex&action=deletehealthcare_new'>Xóa chỉ mục <?=$value?></a>
                    <?php endif ?>
            <?php endforeach ?>
    </div>
    <!-- <div class='card-body'>
