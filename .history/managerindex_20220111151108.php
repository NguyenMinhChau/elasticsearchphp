<?php
    ...
    //check chỉ mục trong mảng listIndexExits đã tồn tại hay chưa
    $listIndexExits = [];
    foreach($listIndexs as $key => $value){
        $listIndexExits[$key] = $client->indices()->exists(['index' => $value]);
    }

    foreach ($listIndexs as $key => $value) {
        if($client->indices()->exists(['index' => $listIndexsExits[$key]])){
