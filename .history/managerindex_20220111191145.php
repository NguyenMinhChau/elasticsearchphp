<?php
    ...
    foreach($listIndexs as $key => $value){
        ...
        if($action == 'delete'.$listsSlug[$key]){
            ...
            //thực hiện tạo index dựa vào listsSlug
        ...
        <div class='alert alert-primary mt-2'>
            <?php 
                if($mgs != ''){
                    ...
                    echo '<p class="text-center">Copyright &copy; <span class="year"></span> all rights reserved.</p>';
                }
            ?>
        </div>
    </div>
                ...
                <?php
                    foreach($listIndexs as $key => $value){
                        ...
                        if($listIndexsExits[$key]){
                            echo '<a href="http://localhost:8800/?page=managerindex&action=delete'.$listsSlug[$key].'" class="btn btn-outline-danger">Xóa chỉ mục</a>';
                        }else{
                            echo '<a href="http://localhost:8800/?page=managerindex&action=create'.$listsSlug[$key].'" class="btn btn-outline-success">Tạo mới chỉ mục</a>';
                        }
                        echo '</td>';
                        echo '</tr>';
                    }
