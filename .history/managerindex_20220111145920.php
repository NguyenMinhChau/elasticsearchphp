    ...
    <div class='card-body'>
        <?php
            foreach ($listIndexs as $key => $value) :?>
                ...
                <?php 
                    echo '<a href="?action=create'.$listsSlug[$key].'" class="btn btn-primary">Tạo mới chỉ mục '.$listIndexs[$key].'</a>';
                    echo '<a href="?action=delete'.$listsSlug[$key].'" class="btn btn-danger">Xóa chỉ mục '.$listIndexs[$key].'</a>';
                ?>
            <?php endforeach ?>
    </div>
</div>