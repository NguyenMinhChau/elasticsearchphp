<?php
    ...
    if($idUpdate != null){
        ...
        $mgs = 'Lấy dữ liệu thành công cho document có id = '.$id;
    }
?>
<div class='card m-4'>
    <h5 class='card-header text-center'>Lấy Document theo ID trong Elasticsearch</h5>
    <div class='card-body'>
...
<div class='card m-4'>
    <h5 class='card-header text-center'>
        <p class="text-left" style='display:inline-block'><button class="btn btn-primary btnRandom">Random ID + ID Doctor</button></p>
        Tạo mới/Cập nhật Document trong Elasticsearch</h5>
    <div class='card-body'>
        <button class="btn btn-primary btnRandom">Random ID + ID Doctor</button>
        <form action='#' method='POST' autocomplete="off" id='form-2' name='form-2'>
            <div class="form-group">
                <label for="id">ID Tài liệu</label>
                <input type="text" class="form-control" id="id" name='id' value='<?=$id?>' disabled placeholder="Fullfluid...">
            </div>
            <div class="form-group">
                <label for="idDoctor">Mã bác sĩ</label>
                <input type="text" class="form-control" id="idDoctor" name='idDoctor' value='<?=$idDoctor?>' disabled placeholder="Fullfluid...">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="slug" name='slug' value="<?=$slug?>" disabled placeholder="Fullfluid...">
            </div>
            <button type="submit" class="btn btn-success btnCreateUpdate">Tạo mới | Cập nhật</button>
        </form>
