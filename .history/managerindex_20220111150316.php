    ...
    <div class='card-body'>
        <?php
            foreach ($listIndexsExits as $key => $value) :?>
                ...
                <?php 
                    if(!$value) :?>
                        <a class='btn btn-primary' href='http://localhost:8800/?page=managerindex&action=create<?=$listsSlug[$key]?>'>Tạo chỉ mục <?=$listIndexs[$key]?></a>
                    <?php else: ?>
                        <a class='btn btn-danger' href='http://localhost:8800/?page=managerindex&action=delete<?=$listsSlug[$key]?>'>Xóa chỉ mục <?=$listIndexs[$key]?></a>
                    <?php endif ?>
            <?php endforeach ?>
    </div>
</div>