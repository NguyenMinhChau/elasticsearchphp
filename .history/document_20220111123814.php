...
<div class='card m-4'>
    ...
    <div class='card-body'>
        <form method='POST' autocomplete='off' id='form-1' name='form-1'>
            ...
            <div class="form-group">
                <label for="idUpdateSelect">ID Tài liệu cập nhật</label>
                <select class="form-control" id="idUpdateSelect" name='idUpdateSelect' value='<?=$id?>'>
                    <option value=''>-- Chọn ID Document --</option>
                    <?php foreach($listId as $id):?>
                        <option value='<?=$idUpdateSelect?>'><?=$id?></option>
                    <?php endforeach;?>
                </select>
                <!-- <input type="text" class="form-control" id="idUpdate" name='idUpdate' value='<?=$idUpdate?>' placeholder='Enter...'/> -->
            </div>
